...
export const getWriteClient = (account) => {
  if (typeof window === 'undefined' || !window.ethereum) {
    throw new Error('MetaMask is required to sign AuditBounty transactions.');
  }
...
export async function loadPrograms(client, fromAddress) {
  let rows = [];
  let listError = null;
  try {
    rows = normalizeProgramList(await readView(client, 'list_programs', [false], fromAddress));
  } catch (err) {
    listError = err;
  }
  ...
  let count = 0;
  try {
    const parsed = parseCount(await readView(client, 'get_program_count', [], fromAddress));
    if (parsed !== null) count = parsed;
  } catch (err) {
    console.warn('get_program_count failed:', err);
  ...
  if (count > 0) {
    ...
    for (let i = 0; i < count; i += 1) {
      try {
        const row = normalizeProgramRow(
          await readView(client, 'get_program', [String(i)], fromAddress),
          String(i)
        );
        if (row) filled.push(row);
      } catch (err) {
        console.warn(`get_program(${i}) failed:`, err);
      }
    }
...
export async function loadReports(client, fromAddress, programFilter = '') {
  let rows = [];
  let listError = null;
  try {
    rows = normalizeReportList(await readView(client, 'list_reports', [programFilter], fromAddress));
  } catch (err) {
    listError = err;
  }
  ...
  let count = 0;
  try {
    const parsed = parseCount(await readView(client, 'get_report_count', [], fromAddress));
    if (parsed !== null) count = parsed;
  } catch (err) {
    console.warn('get_report_count failed:', err);
    return { rows, count: 0, listError: listError || err };
  }
    ...
    for (let i = 0; i < count; i += 1) {
      try {
        const row = normalizeReportRow(
          await readView(client, 'get_report', [String(i)], fromAddress),
          String(i)
        );
        if (row && (!programFilter || String(row.program_id) === String(programFilter))) {
          filled.push(row);
        }
      } catch (err) {
        console.warn(`get_report(${i}) failed:`, err);
      }
    }
    if (filled.length) rows = filled;
  }
